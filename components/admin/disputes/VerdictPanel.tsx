...
export function VerdictPanel({ disputeId, status }: VerdictPanelProps) {
    const [isPending, setIsPending] = useState(false);
    const [notes, setNotes] = useState("");

    const handleResolve = async (verdict: 'student_fault' | 'instructor_fault' | 'force_majeure') => {
        if (!confirm("Esta ação é irreversível. Deseja prosseguir?")) return;

        setIsPending(true);
        try {
            await resolveDispute(disputeId, verdict, notes);
        } catch (error) {
            console.error(error);
            ...
            <div className="bg-[#111a22] rounded-xl border border-[#324d67] p-4 flex flex-col gap-2 shadow-lg">
                ...
                {isResolved ? (
                    <div className="bg-green-500/10 border border-green-500/20 rounded-lg p-4 text-center">
                        <p className="text-green-400 font-bold">Disputa Resolvida</p>
                    </div>
                ) : (
                    <>
                        ...
                        <button
                            onClick={() => handleResolve('student_fault')}
                            disabled={isPending}
                            className="group relative overflow-hidden rounded-lg bg-[#1e293b] border border-[#324d67] hover:border-[#137fec] p-4 text-left transition-all hover:shadow-[0_0_20px_rgba(19,127,236,0.15)] disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                        ...
                        <button
                            onClick={() => handleResolve('instructor_fault')}
                            disabled={isPending}
                            className="group relative overflow-hidden rounded-lg bg-[#1e293b] border border-[#324d67] hover:border-red-500 p-4 text-left transition-all hover:shadow-[0_0_20px_rgba(239,68,68,0.15)] disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                        ...
                        <button
                            onClick={() => handleResolve('force_majeure')}
                            disabled={isPending}
                            className="group relative overflow-hidden rounded-lg bg-[#1e293b] border border-[#324d67] hover:border-slate-400 p-4 text-left transition-all hover:shadow-[0_0_20px_rgba(148,163,184,0.15)] disabled:opacity-50 disabled:cursor-not-allowed"
                        >
            ...
            <div className="bg-[#111a22] rounded-xl border border-[#324d67] p-4 flex-1">
                ...
                {!isResolved && (
                    <div className="flex justify-end mt-2">
                        <button className="text-xs text-white bg-[#324d67] hover:bg-[#4b6a88] px-3 py-1.5 rounded transition-colors disabled:opacity-50">
                            Salvar Nota
                        </button>
                    </div>
                )}
